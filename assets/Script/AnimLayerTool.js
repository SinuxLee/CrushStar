...
const DEVICE_WIDTH = GameConfig.DEVICE_WIDTH
const DEVICE_HEIGHT = GameConfig.DEVICE_HEIGHT
const MoveButtonAnimType = {
  up: 0, // 向上
  down: 1, // 向下
  left: 2, // 向左
  right: 3, // 向右
...
const AnimLayerTool = {
  ...
  bottonAnim: function (button) { // 创建按钮特效
    ...
    for (let i = 0; i < arrayNode.length; i++) {
      ...
      const width = arrayNode[i].height / 20.0
      const anim1 = cc.jumpBy(time, cc.v2(width, 0), width, 1)
      const anim2 = cc.jumpBy(time, cc.v2(-width, 0), -width, 1)
      const anim3 = cc.scaleBy(0.3, 1.1, 0.9)
      const anim4 = cc.delayTime(time)
      const actions = cc.sequence(anim1, anim2, anim3, anim3.reverse(), anim4)
  ...
  createShowMessageBoxAward (parentNode, engineType) // 创建奖励消息提示框
  {
    ...
    const action1 = cc.fadeIn(0.5)
    const action2 = cc.delayTime(1)
    const action3 = cc.fadeOut(0.5)
    const moveFinish = cc.callFunc(this.callFuncAddScore, this, message)
    const action4 = cc.sequence(action1, action2, action3, moveFinish)
    message.runAction(action4)
  },
  moveButtonAnim: function (button, isShow, moveButtonAnimType) { // 按钮消失或出现动画
    const positionX = button.x
    const positionY = button.y
    ...
    if (isShow) {
      ...
      const anim1 = cc.moveTo(this.moveButtonAnimTime, cc.v2(positionX, positionY))
  ...
  {
    const addScoreSprite = new cc.Node()
    const lable = addScoreSprite.addComponent(cc.Label)
    ...
    const move1 = cc.moveBy(0.3, 0, 106)
    const move2 = cc.fadeIn(0.2)
    const move3 = cc.spawn(move1, move2)
    const moveFinish = cc.callFunc(this.callFuncAddScore, this, addScoreSprite)
  ...
  createScoreMoveAnim: function (from, score, scoreType) { // 创建得分移动动画
    if (score > 0) {
      const scoreNumberTTF = new cc.Node()
      const lable = scoreNumberTTF.addComponent(cc.Label)
      lable.font = GameTools.numberLabelAtlas
      lable.string = ':' + score
      scoreNumberTTF.setPosition(from.x, from.y)
      from.getParent().addChild(scoreNumberTTF)
      const moveFinish = cc.callFunc(this.callFuncScoreMoveAnim, this, scoreNumberTTF)
      let move1 = null
      if (scoreType) {
  ...
  {
    const cardSprite = CardSprite.createCardSprite(card.getNumber(), card.getPositionX(), card.getPositionY())
    card.getParent().addChild(cardSprite)
    card.active = false
    card.setNumber(num)
    cardSprite.setNumber(num)

    const action2 = cc.scaleTo(0.3, 0)
    action2.easing(cc.easeBackIn())
    const action3 = cc.scaleTo(0.1, 1)
    const moveFinish1 = cc.callFunc(this.callFuncChangeCardNum1, this, cardSprite)
    const moveFinish2 = cc.callFunc(this.callFuncChangeCardNum2, this, [cardSprite, card])
    const actions = cc.sequence(action2, moveFinish1, action3, moveFinish2)
  ...
  {
    const cardSprite = funData[0]
    const card = funData[1]
    card.CardShow()
    card.active = true
    cardSprite.destroy()
  },
  createExchangeCardAnim: function (card, z, x) // 创建道具交换特效
  {
    ...
    const unitSize = GameConfig.CARD_WIDTH / (GameConfig.CAED_LINES + 1)
    const move1 = cc.scaleBy(0.4, 1.15)
    const actions = cc.sequence(move1, move1.reverse())
    if (z > 0) {
      const card1 = CardSprite.createCardSprite(-1, X - unitSize - GameConfig.CARD_WIDTH, Y)
      card.getParent().addChild(card1)
      card1.runAction(actions.clone().repeatForever())
    }
    if (z < GameConfig.CAED_LINES - 1) {
      const card2 = CardSprite.createCardSprite(-1, X + unitSize + GameConfig.CARD_WIDTH, Y)
      card.getParent().addChild(card2)
      card2.runAction(actions.clone().repeatForever())
    }
    if (x > 0) {
      const card3 = CardSprite.createCardSprite(-1, X, Y - unitSize - GameConfig.CARD_WIDTH)
      card.getParent().addChild(card3)
      card3.runAction(actions.clone().repeatForever())
    }
    if (x < GameConfig.CAED_LINES - 1) {
      const card4 = CardSprite.createCardSprite(-1, X, Y + unitSize + GameConfig.CARD_WIDTH)
  ...
  createScaleToCard: function (card) { // 创建卡片初始化动画
    const cardSprite = CardSprite.createCardSprite(card.number, card.x, card.y)
    card.getParent().addChild(cardSprite)
    card.active = false
    cardSprite.setScale(0)
    const action2 = cc.scaleTo(0.3, 1)
    action2.easing(cc.easeBackOut())
    const moveFinish = cc.callFunc(this.callFuncCard1, this, [cardSprite, card])
  ...
  createMoveAnim: function (from, to, isShowAnim) {
    const cardSprite = CardSprite.createCardSprite(from.number, from.x, from.y)
    to.getParent().addChild(cardSprite)
    const i = 0
    const moveFinish = cc.callFunc(this.callFuncCard2, this, [cardSprite, to, isShowAnim])
  ...
  callFuncCard2: function (sender, to) {
    ...
    if (to[2]) {
      ...
      to[0].CardShow()
      const scale = cc.scaleBy(0.02, 1.15)
      const moveFinish = cc.callFunc(this.callFuncCard3, this, to[0])
  ...
  {
    const moveFinish2 = cc.callFunc(this.callFuncPopStarAnim, this, from)
  ...
  callFuncPopStarAnim (sender, from) // 卡片爆炸特效监听
  {
    ...
    const emitterNode = new cc.Node()
    emitterNode.setPosition(from.getPosition())
    const move_emitter = emitterNode.addComponent(cc.ParticleSystem)
